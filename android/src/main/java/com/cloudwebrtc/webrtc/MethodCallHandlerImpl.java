...
import com.cloudwebrtc.webrtc.record.DunpAudioFrameCapturer;
...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
    ...
    public void onMethodCall(MethodCall call, @NonNull Result notSafeResult) {
        ...
        switch (call.method) {
            case "dunpCaptureFrameOfCurrentAudioStream":
                //peerConnectionId
                Log.i("dunpCaptureFrameOfCurrentAudioStream", "called");
                DunpAudioFrameCapturer.Start(AudioChannel.INPUT, 1000);
                DunpAudioFrameCapturer.Start(AudioChannel.OUTPUT, 1001);
                result.success(null);
                Log.i("dunpCaptureFrameOfCurrentVideoStream", "done");
                break;
    ...
    public String peerConnectionInit(ConstraintsMap configuration, ConstraintsMap constraints) {
        ...
        JavaAudioDeviceModule javAudioDeiveModule = (JavaAudioDeviceModule) audioDeviceModule;

        DunpAudioFrameCapturer.Init(getUserMediaImpl.inputSamplesInterceptor,
                new com.cloudwebrtc.webrtc.record.OutputAudioSamplesInterceptor(javAudioDeiveModule),
                javAudioDeiveModule, peerConnectionId, peerConnection, messenger);
